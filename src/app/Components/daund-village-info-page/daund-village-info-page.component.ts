...
@Component({
  selector: 'app-daund-village-info-page',
  templateUrl: './daund-village-info-page.component.html',
  styleUrls: ['./daund-village-info-page.component.css']
})
export class DaundVillageInfoPageComponent {
  ...
  ngOnInit() {
    ...
    this.fetchDaundVillagebyID();

  }

  fetchDaundVillagebyID() {
    const villageId = this.route.snapshot.paramMap.get('id');
    // Fetch village data from the API based on the villageId
    this.http.get<any>(`${this.url}/daundvillagedetails/${villageId}/`)
      .subscribe(data => {
        console.log("Daund Data", data.village_data);
