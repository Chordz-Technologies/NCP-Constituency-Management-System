...
@Component({
  selector: 'app-daund-map',
  templateUrl: './daund-map.component.html',
  styleUrls: ['./daund-map.component.css']
})
export class DaundMapComponent {
  ...
  fetchSvgInfo() {
    this.dataService.getAllDataD().subscribe(data => {
  ...
  openModal(regionId: number, content: any) {
    this.http.get<any>(`${this.url}/daundvillagedetails/${regionId}/`)
      .subscribe(data => {
  ...
  navigateToVillageDetails(modal: any) {
    if (this.villageData) {
      ...
      this.router.navigate(['/daund_village_information', this.villageData.id]);
    }
  }
}
