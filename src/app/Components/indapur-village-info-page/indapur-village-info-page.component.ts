...
@Component({
  selector: 'app-indapur-village-info-page',
  templateUrl: './indapur-village-info-page.component.html',
  styleUrls: ['./indapur-village-info-page.component.css']
})
export class IndapurVillageInfoPageComponent {
  ...
  ngOnInit() {
    ...
    this.fetchIndapurVillagebyID();

  }

  fetchIndapurVillagebyID() {
    const villageId = this.route.snapshot.paramMap.get('id');
    // Fetch village data from the API based on the villageId
    this.http.get<any>(`${this.url}/indapurvillagedetails/${villageId}/`)
      .subscribe(data => {
        console.log("Indapur Data", data.village_data);
